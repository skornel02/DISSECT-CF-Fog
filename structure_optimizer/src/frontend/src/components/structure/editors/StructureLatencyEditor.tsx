...
export default function StructureLatencyEditor({
  ...
  const regionConnections = useMemo(
    () => structure.regionConnections === undefined ? [] : [...structure.regionConnections],
        ...
        <EditableTableCell
          ...
          setValue={(value) => {
            ...
            if (Number.isSafeInteger(parseInt(value))) {
              console.log("Setting latency", row.original.region, region.name, parseInt(value));
              setStructure((_) => ({
                ..._,
                regionConnections: [
                  ...regionConnections.filter((r) => !(r.from === row.original.region && r.to === region.name)),
                  {
              ...
              setStructure((_) => ({
                ..._,
                regionConnections: regionConnections.filter((r) => !(r.from === row.original.region && r.to === region.name)),
              }));
            }
          }}
        />
  ...
  ] satisfies ColumnDef<RegionLatencyType>[], [regions, regionConnections]);
  ...
  const regionRows = useMemo(() => {
    return regions.map(region => {
      ...
      regions.forEach(region2 => {
        const connection = regionConnections.find((r) => r.from === region.name && r.to === region2.name);
